            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        @endif
                        <div class="card mb-4">
                            <div class="card-header text-primary"> تعديل ميعاد الامتحان </div>
                            <div class="card-body">
                                <form method="post" action="{{ route('test-groups.update', $test_group->id) }}" class="needs-validation"  id="testGroupCreate">
                                    @csrf
                                    @method('put')
                                    <div class="form-row">
                                        <div class="col-md-6 form-group">
                                            <label class="s" for="test-options"> اسم الامتحان </label>
                                            <input class="form-control" value="{{ $test_group->test->name }}">
                                            <input class="form-control" name="test_id" value="{{ $test_group->test_id }}" hidden>
                                        ...
                                        <div class="form-row ">
                                            <div class="col-sm-6 col-md-2 form-group" id="form-group">
                                                ...
                                                <div class="input-group-append">
                                                    <input  id="date-1" name="date[day]" class="form-control datetimepicker" value="{{ $test_group->times[0]->day }}"  placeholder="تاريخ الامتحان"    type="text" >
                                            ...
                                            <div class="col-sm-6 col-md-2 form-group" id="form-group">
                                                ...
                                                <select name="date[begin]" class="form-control times "  id="begin-1">
                                                    @foreach(App\Time::hours() as $hour)
                                                        <option value="{{ $hour }}" {{ $hour == $test_group->times[0]->begin? 'selected':''}}>{{ $hour }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-sm-6 col-md-2 form-group" id="form-group">
                                                ...
                                                <select  name="date[end]" class="form-control times"  id="end-1">
                                                    @foreach(App\Time::hours() as $hour)
                                                        <option value="{{ $hour }}" {{ $hour == $test_group->times[0]->end? 'selected':'' }}>{{ $hour }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-sm-6 col-md-2 form-group" id="form-group">
                                                ...
                                                <select name="room" class="form-control "  id="room-1">
                                                    <option value="{{ $test_group->times[0]->pivot->room_id }}">{{ App\Room::findOrFail( $test_group->times[0]->pivot->room_id )->name }}</option>
                                                </select>
                                            </div>

                                            <div class="col-sm-6 col-md-1 form-group" id="form-group">
                                                <label for="test_time[1][seats]">  عدد المقاعد  </label>
                                                <input type="number" name="available_chairs" value="{{ $test_group->available_chairs }}" class="form-control char"  id="seat-1">
                                            </div>
                                        </div>
                                    </div>
